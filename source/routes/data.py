...
async def upload_data(
    ...
    # vaildate the file uploaded
    data_controller = DataController()
    is_vaild, result_signal = data_controller.validate_uploaded_file(file=file)
...
async def process_endpoint(request: Request, project_id: str, process_request: ProcessRequest):
    ...
    if process_request.file_id:
        ...
        if asset_record is None:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponseSignal.ASSET_ID_ERROR.value,
                }
            )
    ...
    if len(project_file_ids) == 0:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "signal": ResponseSignal.NO_ASSETS_FOUND.value,
            }
        )
    ...
    return JSONResponse(
        content={
            ...
            "processed_assets": number_of_assets
        }
    )
